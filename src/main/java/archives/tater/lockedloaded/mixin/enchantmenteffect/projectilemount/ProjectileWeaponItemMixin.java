...
public class ProjectileWeaponItemMixin {
    ...
    private <T extends Projectile> T setProjectileMountable(T original, ServerLevel level, LivingEntity shooter, InteractionHand hand, ItemStack weapon) {
        EnchantmentHelper.runIterationOnItem(weapon, (enchantment, enchantmentLevel) -> {
            Enchantment.applyEffects(
                    enchantment.value().getEffects(LockedLoadedEnchantmentEffects.PROJECTILE_MOUNTABLE),
                    LockedLoadedLootContext.projectileContext(level, enchantmentLevel, original),
                    _ -> original.setAttached(LockedLoadedAttachments.MOUNTABLE_PROJECTILE, Unit.INSTANCE)
            );
        });
